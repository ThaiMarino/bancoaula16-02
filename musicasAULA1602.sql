...
SELECT *
FROM clientes
inner JOIN planos ON planos.id = clientes.planos_id;



SELECT descricao AS 'Nome do Plano',
...
SELECT descricao AS 'Nome Plano',
    valor,
    limite
FROM planos
ORDER BY valor;
...
FROM artistas
inner JOIN gravadoras ON artistas.gravadoras_id = gravadoras.id
GROUP BY gravadoras.id;

-- 6) Qual gravadora tem mais artistas?

SELECT gra.nome as 'Nome Gravadora', count(art.id) as 'Total Artistas' FROM artistas art
INNER JOIN gravadoras gra
ON art.gravadoras_id = gra.id
GROUP BY gra.nome 
...
select MU.nome, AR.nome
FROM musicas MU
INNER JOIN musicas_has_artistas MA
ON MU.id = MA.musicas_id
INNER JOIN artistas AR
ON MA.artistas_id = AR.id
WHERE AR.nome = "Mano Lima";


-- 8) Quais são as músicas da gravadora ACIT?

SELECT gravadoras.nome AS 'Nome Gravadora', musicas.nome AS 'Nome Músicas'
FROM musicas_has_artistas AS mha
INNER JOIN musicas ON mha.musicas_id = musicas.id
INNER JOIN artistas ON mha.artistas_id = artistas.id
INNER JOIN gravadoras ON artistas.gravadoras_id = gravadoras.id
...
SELECT generos.descricao AS 'Gênero',
COUNT(musicas.id) AS 'Total de Músicas'
FROM generos
INNER JOIN musicas ON generos.id = musicas.generos_id
group by generos.id
ORDER BY 1;
...
SELECT clientes.login AS 'Cliente',
COUNT(mhc.musicas_id) AS 'Qtde Baixada'
FROM musicas_has_clientes AS mhc
INNER JOIN clientes
ON mhc.clientes_id = clientes.id
GROUP BY 1
ORDER BY 1;
